...
bool SYDirFileManger::EnumDirPlusWithDirPath(const WCHAR *pPath, std::vector<SYDirFileInfo*>* pVecPlusEnum)
{
	...
	*/
	...
	HANDLE hFile = FindFirstFile( strPath.c_str(), &wfd );
...
bool SYDirFileManger::DeleteFileWithPath(const WCHAR *pPath)
{
	if (DeleteFile(pPath))
		return true;
	return false;
...
bool SYDirFileManger::DeleteDirectoryWithPath(const WCHAR *pPath, bool noRecycleBin)
{
